...
\section{Methods}\label{irgendwie}
...
over the data.
\item{multithread:} Distribute the algorithm among all CPU cores to speed up the computation (not available on Windows).
\end{itemize}
...
\emph{every} row of the provided template. The results will be normalised against this control.
\end{itemize}
