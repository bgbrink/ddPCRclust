...
ddPCRclust <-
  function(files,
    ...
    if (!is.null(template)) {
      ...
      if (substr(header, start = 1, stop = 1) == ">") {
        ...
        if (length(annotations) < 4) {
          warning(
            "Missing fields in header. Please use the following layout: experiment_name, ch1, ch2, descriptions"
          )
          annotations <- NULL
        } else {
          ...
          if (any(is.na(c(ch1, ch2)))) {
            warning(
              "Unknown fields in header. Please use the following layout: experiment_name, ch1, ch2, descriptions"
            )
            annotations <- NULL
          } else {
            annotations <-
              c(annotations[1],
                ch1,
                ch2,
                paste(annotations[4:length(annotations)], collapse = ","))
            names(annotations) <-
              c("Name", "Ch1", "Ch2", "Descriptions")
          }
        }
        template <- utils::read.csv(template, skip = 1)
        numOfMarkers <-
          lapply(ids, function(x) {
            unlist(template[which(template[, 1] == x), 3])
          })
        markerNames <-
          lapply(ids, function(x) {
            unlist(template[which(template[, 1] == x), 4:7])
          })
      } else {
        stop(paste(
          "Invalid Template file! This file starts with:\n",
    ...
    dens_result <-
      sam_result <- peaks_result <- rep(0, length(files))
    names(dens_result) <-
      names(sam_result) <- names(peaks_result) <- ids
    if (length(files) > 1) {
      ...
      dens_result <-
        mcmapply(
          ...
          similarityParam = similarityParam,
      ...
      superResults <- list()
      superResults[[ids]] <-
        ensemble_wrapper(dens_result, sam_result, peaks_result, csvFiles)
    }
    ...
    for (i in seq_along(data)) {
      ...
      if (invert) {
        p <-
          ggplot(
            data = result$data,
            mapping = aes_(x = ~ Ch2.Amplitude, y = ~ Ch1.Amplitude)
          )
      } else {
        p <-
          ggplot(
            data = result$data,
            mapping = aes_(x = ~ Ch1.Amplitude, y = ~ Ch2.Amplitude)
          )
      }
      ...
      if (invert) {
        p <-
          p + geom_point(aes(color = factor(Cluster)), size = .4, na.rm = TRUE) + ggtitle(id) + theme_bw() + theme(legend.position =
                                                                                                                     "none") +
          scale_colour_manual(values = cbPalette) + labs(
            x = paste(annotations[3], "Amplitude"),
            y = paste(annotations[2], "Amplitude")
        ...
        p <-
          p + geom_point(aes(color = factor(Cluster)), size = .4, na.rm = TRUE) + ggtitle(id) + theme_bw() + theme(legend.position =
                                                                                                                     "none") +
          scale_colour_manual(values = cbPalette) + labs(
            x = paste(annotations[2], "Amplitude"),
            y = paste(annotations[3], "Amplitude")
          ) + theme(axis.text = element_text(size = 12),
                    axis.title = element_text(size = 14))
      }
      ggsave(paste0(directory, "/", annotations[1], "/", id, ".", format),
             p,
             dpi = 1200)
    }
  }
...
#'
exportToExcel <-
  function(data, directory, annotations, raw = FALSE) {
    directory <- normalizePath(directory, mustWork = TRUE)
    ifelse(!dir.exists(paste0(directory, "/", annotations[1])), dir.create(paste0(directory, "/", annotations[1])), FALSE)
    dataToWrite <- NULL
    for (i in seq_along(data)) {
      ...
      if (is.null(dataToWrite)) {
        ...
        dataToWrite <-
          rbind(dataToWrite, tmp[match(colnames(dataToWrite), colnames(tmp))])
      } else {
    ...
    dataToWrite <-
      t(dataToWrite[, match(mynames, colnames(dataToWrite))])
...
ensemble_wrapper <-
  function(dens_result,
    ...
    result <-
      tryCatch(
        createEnsemble(dens_result, sam_result, peaks_result, file[, c(2, 1)]),
        error = function(e) {
          print(e)
        }
      )
...
runDensity <-
  function(file,
    ...
    if (length(secondClusters$clusters) > 0) {
      posOfSeconds <-
        (utils::tail(posOfFirsts, 1) + 1):(utils::tail(posOfFirsts, 1) + nrow(secondClusters$clusters))
      ClusterCentres <-
        rbind(ClusterCentres, secondClusters$clusters)
    }
    if (length(tertClusters$clusters) > 0) {
      posOfThirds <-
        (utils::tail(posOfSeconds, 1) + 1):(utils::tail(posOfSeconds, 1) + nrow(tertClusters$clusters))
      ClusterCentres <- rbind(ClusterCentres, tertClusters$clusters)
    }
    if (length(quadCluster$clusters) > 0) {
      posOfFourth <-
        max((utils::tail(posOfFirsts, 1) + 1),
            (utils::tail(posOfSeconds, 1) + 1),
            (utils::tail(posOfThirds, 1) + 1),
            na.rm = TRUE)
      ClusterCentres <-
    ...
    for (c in seq_len(nrow(ClusterCentres))) {
      result[as.numeric(rownames(newData[(
        newData[, 1] < ClusterCentres[c, 1] + scalingParam[1] &
          newData[, 1] > ClusterCentres[c, 1] - scalingParam[1]
        &
          newData[, 2] < ClusterCentres[c, 2] + scalingParam[2] &
          newData[, 2] > ClusterCentres[c, 2] - scalingParam[2]
      ), ]))] <- c
      newData <-
        subset(
    ...
    return(
      list(
        data = result,
        counts = NumOfEventsClust,
        firstClusters = firstClusters$clusters,
        partition = partition
      )
    )
  }
...
runSam <-
  function(file,
    ...
    samRes <-
      SamSPECTRAL(
        data.points = as.matrix(file),
        dimensions = c(1, 2),
        normal.sigma = (200 * sensitivity ^ 2),
        separation.factor = (0.88 * sensitivity),
        m = m,
        talk = FALSE
      )
    data <- file
    temp <-
      lapply(min(samRes, na.rm = TRUE):max(samRes, na.rm = TRUE), function(x)
    ...
    secondaryClusters <-
      tertiaryClusters <- quaternaryCluster <- NULL
    firstClusters <-
      findPrimaryClusters(
        samRes,
        clusterMeans,
    ...
    if (length(firstClusters) == 2) {
      quaternaryCluster <-
        findQuaternaryCluster(clusterMeans, emptyDroplets, badClusters, firstClusters)
      samResult <-
        c(emptyDroplets, firstClusters, quaternaryCluster)
    }
    ...
    if (length(firstClusters) == 4) {
      ...
      tertiaryClusters <-
        findTertiaryClusters(
          ...
          badClusters,
          clusterMeans,
          secondaryClusters$correctionFactor,
          sum(dimensions),
          samTable
        )
      quaternaryCluster <-
    ...
    samRes <-
      mergeClusters(samRes, clusterMeans, samResult, badClusters)
    rain <-
      assignRain(
        clusterMeans,
        data,
  ...
  function(file,
    ...
    fPeaksRes <-
      flowPeaks(
        file,
        tol = 0,
        h0 = (0.3 / sensitivity ^ 1.5),
        h = (0.5 / sensitivity ^ 1.5)
      )
    data <- file
  ...
  function(results,
    ...
    for (i in seq_along(results)) {
      ...
      if (is.null(result))
        next
      markers <-
        as.character(unlist(template[which(template[, 1] == id), 4:7]))
      ...
      for (j in seq_len(4)) {
        ...
        countedResult[[id]][[marker]] <-
          list(counts = counts, cpd = cpd)
      }
    ...
    if (!is.null(constantControl)) {
      for (i in seq_along(countedResult)) {
        ...
        for (j in markers) {
          if (j == "")
            next
          countedResult[[id]][[j]]$cpd <-
            countedResult[[id]][[j]]$cpd * modFactor
        }
      }
    }
  ...
  function(dens = NULL,
    ...
    return(
      list(
        ...
        confidence = conf,
        counts = superCounts,
        nrOfAlgorithms = length(listResults)
      )
    )
  }
...
shearCorrection <- function(counts, lengthControl, stableControl) {
  ...
  for (well in counts) {
    ...
    if (length(markerPos) == 0 || length(controlPos) == 0 ||
        length(well[[controlPos]]$cpd) == 0 ||
        well[[controlPos]]$cpd == 0)
      next
