...
dens_wrapper <-
  function(file,
    ...
    result <-
      tryCatch(
        expr = withTimeout(
          runDensity(file[, c(2, 1)], sensitivity, numOfMarkers, missingClusters),
          timeout = 60
        ),
...
sam_wrapper <-
  function(file,
    ...
    result <-
      tryCatch(
        expr = withTimeout(
          runSam(file[, c(2, 1)], sensitivity, numOfMarkers, missingClusters),
          timeout = 60
        ),
        TimeoutException = function(ex)
          "TimedOut",
...
peaks_wrapper <-
  function(file,
    ...
    result <-
      tryCatch(
        expr = withTimeout(
          runPeaks(file[, c(2, 1)], sensitivity, numOfMarkers, missingClusters),
          timeout = 60
        ),
        TimeoutException = function(ex)
          "TimedOut",
  ...
  function(file,
    ...
    samRes <-
      SamSPECTRAL(
        ...
        dimensions = c(1, 2),
        normal.sigma = (400 * sensitivity ^ 2),
