  ...
  function(f,
    ...
    if (length(Xc) == 4 && length(Yc) == 4) {
      ...
      test_left <-
        abs(det(rbind(
          cbind(1, 1, 1),
          cbind(Rot_xy_leftPrim, Rot_xy_midLeftPrim, rbind(0, 0))
        ))) / 100
      test_right <-
        abs(det(rbind(
          cbind(1, 1, 1),
    ...
    return(list(
      clusters = cbind(Xc, Yc),
      deviation = cbind(deviationX, deviationY)
    ))
  }
...
findSecondaryClustersDensity <-
  function(f,
      ...
      if (length(Cuts) == (choose(NumberOfSinglePos, 2) - 1)) {
        ...
        for (r1 in 1:(length(Cuts) - 1)) {
          ...
          switch(r1,
                 {
                   XxA <-
                     (firstClusters$clusters[1, 1] + firstClusters$clusters[2, 1] - emptyDroplets[1])
                   YyA <-
                     (firstClusters$clusters[1, 2] + firstClusters$clusters[2, 2] - emptyDroplets[2])
                 },
                 {
                   XxA <-
                     (firstClusters$clusters[1, 1] + firstClusters$clusters[3, 1] - emptyDroplets[1])
                   YyA <-
                     (firstClusters$clusters[1, 2] + firstClusters$clusters[3, 2] - emptyDroplets[2])
                 },
                 {
                   XxA <-
                     (firstClusters$clusters[2, 1] + firstClusters$clusters[3, 1] - emptyDroplets[1])
                   YyA <-
                     (firstClusters$clusters[2, 2] + firstClusters$clusters[3, 2] - emptyDroplets[2])
                 },
                 {
                   XxA <-
                     (firstClusters$clusters[1, 1] + firstClusters$clusters[4, 1] - emptyDroplets[1])
                   YyA <-
                     (firstClusters$clusters[1, 2] + firstClusters$clusters[4, 2] - emptyDroplets[2])
                 },
                 {
                   XxA <-
                     (firstClusters$clusters[2, 1] + firstClusters$clusters[4, 1] - emptyDroplets[1])
                   YyA <-
                     (firstClusters$clusters[2, 2] + firstClusters$clusters[4, 2] - emptyDroplets[2])
                 },
                 {
                   XxA <-
                     (firstClusters$clusters[3, 1] + firstClusters$clusters[4, 1] - emptyDroplets[1])
                   YyA <-
                     (firstClusters$clusters[3, 2] + firstClusters$clusters[4, 2] - emptyDroplets[2])
                 })
          if (length(indices) < 2) {
            deviation2X <-
          ...
          repeat {
            ...
            if ((tinyPXx + epsilon) > highPXx) {
              Xx <- Xx[1]
              message(paste0(
                "Error finding X value for peak ",
                r1,
                " of ",
          ...
          repeat {
            ...
            if ((tinyPYy + epsilon) > highPXx) {
              Yy <- Yy[1]
              message(paste0(
                "Error finding Y value for peak ",
                r1,
                " of ",
                choose(NumberOfSinglePos, 2)
              ))
        ...
        for (r1 in 1:NumberOfSinglePos) {
          for (r2 in 1:NumberOfSinglePos) {
            if (r1 >= r2)
              next
            Xx <-
              firstClusters$clusters[r1, 1] + firstClusters$clusters[r2, 1] - emptyDroplets[1]
            Yy <-
    ...
    if (length(Xc2g) > 3 &&
        (Xc2g[3] ^ 2 + Yc2g[3] ^ 2) >= 1.2 * (Xc2g[4] ^ 2 + Yc2g[4] ^ 2)) {
        ...
        switch(r1,
               {
                 XxA <-
                   mean(
                     secondaryClusters$clusters[1, 1] + firstClusters$clusters[3, 1] - emptyDroplets[1],
                 ...
                 YyA <-
                   mean(
                     secondaryClusters$clusters[1, 2] + firstClusters$clusters[3, 2] - emptyDroplets[2],
               ...
               },
               {
                 XxA <-
                   mean(
                     secondaryClusters$clusters[1, 1] + firstClusters$clusters[4, 1] - emptyDroplets[1],
                 ...
                 YyA <-
                   mean(
                     secondaryClusters$clusters[1, 2] + firstClusters$clusters[4, 2] - emptyDroplets[2],
               ...
               },
               {
                 XxA <-
                   mean(
                     secondaryClusters$clusters[2, 1] + firstClusters$clusters[4, 1] - emptyDroplets[1],
                 ...
                 YyA <-
                   mean(
                     secondaryClusters$clusters[2, 2] + firstClusters$clusters[4, 2] - emptyDroplets[2],
               ...
               },
               {
                 XxA <-
                   mean(
                     secondaryClusters$clusters[3, 1] + firstClusters$clusters[4, 1] - emptyDroplets[1],
                 ...
                 YyA <-
                   mean(
                     secondaryClusters$clusters[3, 2] + firstClusters$clusters[4, 2] - emptyDroplets[2],
  ...
  function(f,
    ...
    if (nrow(f_onlyQuad) >= 4) {
      ...
      if (is.null(tertiaryClusters)) {
        ...
        XcQuad <-
          firstClusters$clusters[1, 1] + firstClusters[2, 1] - emptyDroplets[1]
        YcQuad <-
          firstClusters$clusters[1, 2] + firstClusters[2, 2] - emptyDroplets[2]
      } else {
        XcQuad <-
          mean(
            tertiaryClusters$clusters[1, 1] + firstClusters$clusters[4, 1] - emptyDroplets[1],
        ...
        YcQuad <-
          mean(
            tertiaryClusters$clusters[1, 2] + firstClusters$clusters[4, 2] - emptyDroplets[2],
...
findPrimaryClusters <-
  function(data,
    ...
    clusterMeans2 <-
      clusterMeans[-c(emptyDroplets, remove), , drop = FALSE]
    ...
    selection <-
      which(clusterMeans[, 1] <= max(clusterMeans[collinear1, 1]))
    ...
    selection <-
      which(clusterMeans[, 2] <= max(clusterMeans[collinear2, 2]))
    selection <-
      selection[!selection %in% c(emptyDroplets, collinear1, remove)]
    selection <-
      selection[(dataTable[selection] > max(dataTable[selection]) / 4)]
    realFirstCluster2 <-
  ...
  function(data,
    ...
    repeat {
      if (length(temp) == 0) {
        ...
        if (dataTable[cluster] < (mean(dataTable[c(realFirstCluster1, realFirstCluster2)])) /
            5) {
...
findSecondaryClusters <-
  function(firstClusters,
        ...
        for (b in firstClusters) {
          ...
          if (sum(abs(distance)) > threshold) {
            ...
            correction[[match(a, firstClusters)]] <-
              c(correction[[match(a, firstClusters)]], list(distance))
            correction[[match(b, firstClusters)]] <-
              c(correction[[match(b, firstClusters)]], list(distance))
          }
        }
      }
        ...
        for (b in firstClusters) {
          if (match(a, firstClusters) >= match(b, firstClusters)) {
            ...
            if (min(rowSums) < threshold) {
              ...
              correction[[match(b, firstClusters)]] <-
                c(correction[[match(b, firstClusters)]], temp[minimum])
    ...
    if ((stats::IQR(counts[secondClusters]) > q[3]) &&
        (q[3] > 5) && (q[1] < 5)) {
      remove <-
        c(remove, as.integer(names(which.min(counts[secondClusters]))))
      return(
        findSecondaryClusters(
          firstClusters,
          clusterMeans,
    ...
    if (max(counts[secondClusters]) > q[4] * 5) {
      remove <-
        c(remove, as.integer(names(which.max(counts[secondClusters]))))
      return(
        findSecondaryClusters(
          firstClusters,
          clusterMeans,
...
findTertiaryClusters <-
  function(emptyDroplets,
    ...
    clusterMeans2 <-
      clusterMeans[-c(emptyDroplets, firstClusters, secondClusters, remove), , drop =
                     FALSE]
    if (length(clusterMeans2) == 0)
    ...
    if ((stats::IQR(counts[thirdClusters]) > q[3]) &&
        (q[3] > 5) && (q[1] < 5)) {
      remove <-
        c(remove, as.integer(names(which.min(counts[thirdClusters]))))
      return(
        findTertiaryClusters(
          emptyDroplets,
          firstClusters,
    ...
    if (max(counts[thirdClusters]) > q[4] * 5) {
      remove <-
        c(remove, as.integer(names(which.max(counts[thirdClusters]))))
      return(
        findTertiaryClusters(
          emptyDroplets,
          firstClusters,
...
findQuaternaryCluster <-
  function(clusterMeans,
    ...
    clusterMeans2 <-
      clusterMeans[-c(emptyDroplets, firstClusters, secondClusters, thirdClusters), , drop =
                     FALSE]
    if (length(clusterMeans2) == 0)
      return(0)
    rowSums <-
...
adjustClusterMeans <-
  function(data, clusterMeans, result, clusters) {
  ...
  function(clusterMeans,
      ...
      for (r in 1:length(minimalDistance)) {
        ...
        if (distPoint <= 0.2 * distTotal) {
          ...
          result[as.numeric(rownames(newData)[r])] <-
            firstClusters[minimalDistance[r]]
        }
      }
    }
      ...
      if (nrow(newData) > 0) {
        ...
        for (r in 1:length(minimalDistance)) {
          ...
          if (distPoint <= 0.2 * distTotal) {
            ...
            result[as.numeric(rownames(newData)[r])] <-
              secondClusters[helper2[minimalDistance[r]]]
          }
        }
      }
    }
      ...
      if (nrow(newData) > 0) {
        ...
        for (r in 1:length(minimalDistance)) {
          ...
          if (distPoint <= 0.2 * distTotal) {
            ...
            result[as.numeric(rownames(newData)[r])] <-
              thirdClusters[helper4[minimalDistance[r]]]
          }
        }
      }
    }
    ...
    if (!is.null(fourthCluster) && sum(fourthCluster) > 0) {
      ...
      for (c in fourthCluster) {
        sdC <-
          2 * c(stats::sd(data[result == c, 1], na.rm = TRUE),
